...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        # loading waypoints
        csv_path = "/home/jaycee/sim_ws/src/csv_data"
        self.map_name = "chill"
        csv_data = np.loadtxt(csv_path + '/' + self.map_name + '.csv', delimiter=',', skiprows=0)  # Usa ',' como delimitador
        self.waypoints = csv_data[:, 0:2] # first row is indices
        self.num_waypoints = self.waypoints.shape[0]
        self.ref_speed = csv_data[:, 2] * 0.7 # max speed - sim is 10m/s, levine 2nd - real is 6m/s
